...
\end{align}
where the last step is by integration by parts \footnote{This trick will be used all over the place when constructing gradients. See \ref{sct: Natural Boundary Condition} in the Appendix}.
...
From (\ref{equ: Dirichlet Differential No IBP}), we use IBP while assuming for conditons that eliminates the boundary terms.

\subsubsection{$L^2$}
Starting from (\ref{equ: Dirichlet Differential No IBP}),
\begin{align}
    -2 d \mathcal{E}_D |_f(u) - \langle \Delta f, u \rangle_{L^2} &= \langle \Delta u, f \rangle_{L^2} \\
    &= \int_{R} f \nabla^2 u \dV \\
    &= \int_{R} u \nabla^2 f + \nabla \cdot \left( f \nabla u - u \nabla f \right) \dV \\
    &= \int_{R} u \nabla^2 f + \oint_{\partial R} \left( f \nabla u - u \nabla f \right) \cdot \mathbf{n} \ds \\
    &= \langle \Delta f, u \rangle_{L^2} + \oint_{\partial R} \underbrace{\left( f \nabla u - u \nabla f \right)\cdot \mathbf{n}}_{\text{Boundary Terms}}  \ds 
\end{align}

\subsubsection{$H^1$}
Starting from (\ref{equ: Dirichlet Differential No IBP}),
\begin{align}
    -2 d \mathcal{E}_D |_f(u) &= \left( \langle \Delta f, u \rangle_{L^2} + \langle \Delta u, f \rangle_{L^2} \right) \\
    &= \int_R \left( u \nabla^2 f + f \nabla^2 u \right) \dV \\
    &= \int_R \nabla \cdot \left( u \nabla^2 f \right) - \nabla u \cdot \nabla f \dV + \int_R \nabla \cdot \left( f \nabla^2 u \right) - \nabla u \cdot \nabla f \dV \\
    &= -2 \int_R \nabla u \cdot \nabla f \dV + \oint_{\partial R} \left( u \nabla^2 f + f \nabla^2 u \right) \cdot \mathbf{n} \ds \\
    &= -2 \langle \nabla u, \nabla f \rangle_{H^1} + \oint_{\partial R} \underbrace{\left( u \nabla^2 f + f \nabla^2 u \right)\cdot \mathbf{n}}_{\text{Boundary Terms}}  \ds 
\end{align}
