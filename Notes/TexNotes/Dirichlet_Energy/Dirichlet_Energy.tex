...
\subsubsection{Dirichlet Boundary Condition (Deprecated)}
...
Assuming periodic boundary condition $f_0^m = f_J^m$,
we may write this as a matrix equation:
\begin{align*}
    &\underbrace{
        \begin{pmatrix}
...
A few remarks:
\begin{itemize}
    \item Because $A$ is diagonally dominant tridiagonal matrix, this matrix equation can be solved using Thomas' algorithm (stably)\footnote{https://en.wikipedia.org/wiki/Tridiagonal\_matrix\_algorithm}
    \item Alternatively, one could write down the inverse matrix explicitly
        (Although, matrix multiplication is usually not stable\dots)
\end{itemize}
...
\end{equation}
Suppose $\alpha_n \equiv 1$ (Otherwise we could absorb it into $\mathcal{E}$, or take a different time scale.), and divide by the time step $\Delta t$, then take the limit as $\Delta t \rightarrow 0$ to arrive at the gradient flow equation.
...
\end{equation}
the $L^1$ norm should be conserved. (I'm guessing when sign doesn't change?)
\begin{proof}
    \begin{align}
        \frac{\partial ||u(\cdot,t)||_{L^1}}{\partial t} &= \frac{\partial}{\partial t} \int_{\Omega} |u(x,t)| \dx \\
        &= \frac{\partial}{\partial t} \int_{\Omega} u(x,t) \dx \\
        &= \int_{\Omega} \frac{\partial u}{\partial t} \dx \\
        &= \int_{\Omega} \nabla \cdot \mathbf{F} \dx \\
        &= \oint_{\partial \Omega} \mathbf{F} \cdot \mathbf{n} \ds
    \end{align}
    With no boundaries, this term should be zero.
\end{proof}
\end{document}
