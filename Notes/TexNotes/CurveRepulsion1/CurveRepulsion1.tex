...
There are a few choices for discretized energy.
The \textbf{4-point quadrature discretized energy} $E$ can be expressed as:
\begin{align}
    E &= \sum_{i = 1}^{J} \sum_{\substack{j = 1 \\ |j-i| > 1}} k_{i,j} \norm{x_{i+1} - x_i} \, \norm{x_{j+1} - x_j} 
    \label{equ: 4-point Discretized Energy}
    \\
...
\begin{figure}[h]
    ...
    \caption{Kernel $k_{i,j}$ for 4-point quadrature discretized energy computation}
\end{figure}

A potential problem is that it loses the sense of an ``edge''.

Alternatively, \textbf{1-point quadrature discretized energy} $E$ can be expressed as:
\begin{align}
    E &= \sum_{i = 1}^{J} \sum_{\substack{j = 1 \\ |j-i| > 1}} k_{i,j} \norm{x_{i+1} - x_i} \, \norm{x_{j+1} - x_j} 
    \label{equ: 1-point Discretized Energy}
    \\
    k_{i,j} &= k_{\beta}^{\alpha} \left( x_{i_m}, x_{j_m}, T_i \right)
\end{align}
where $x_{i_m}$ and $x_{j_m}$ are the points that have the minimal distance out of the four corners.
...
Note that in the case that we use (\ref{equ: 4-point Discretized Energy}),
...
Given (\ref{equ: 4-point Discretized Energy}), instead of approximating the differential by central scheme,
