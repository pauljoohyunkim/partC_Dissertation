...
__global__ static void repulsiveCurveDifferential(double* dev_x, double* dev_y, double* dev_z, double* dev_energyMatrixFlattened, double J)
{
    ...
    if (i < J)
    {
        ...
        if (j == 0)
        {
            cuDifferentialTangentMinimal(dev_x, dev_y, dev_z, i, DELTA_X, 0.0, 0.0, J, &dev_energyMatrixFlattened[flattenedPos]);
        }
        if (j == 1)
        {
            cuDifferentialTangentMinimal(dev_x, dev_y, dev_z, i, 0.0, DELTA_X, 0.0, J, &dev_energyMatrixFlattened[flattenedPos]);
        }
        if (j == 2)
        {
            cuDifferentialTangentMinimal(dev_x, dev_y, dev_z, i, 0.0, 0.0, DELTA_X, J, &dev_energyMatrixFlattened[flattenedPos]);
        }
    }
}
