...
\begin{lemma}
    Given $\alpha$ and $\beta$, the behaviour of the kernel at a point and its perturbation of order $O \left( \epsilon \right)$ is of order $O \left( \epsilon^{2\alpha - \beta} \right)$. Moreover, if $2\alpha = \beta$, then the kernel converges to $\left( \frac{\kappa}{2} \right)^{\alpha}$ as the two points get closer, where $\kappa$ is the curvature of the curve at the point.
    \begin{proof}
