...
\begin{figure}[tbp]
    ...
    \begin{subfigure}[b]{0.75\textwidth}
        ...
        \caption{For a chosen edge $e_i^k$, we ignore the two adjacent edges $e_{i-1}^k$, $e_{i+1}^k$.
...
$\Grad_{\mathcal{X}}$ is discrete equivalent\footnote{Worth noting that $\Grad_{\mathcal{X}}:\mathbb{T} \rightarrow \mathbb{T}$ where $\mathbb{T}$ is a set of arrays of certain shape; $\Grad_{\mathcal{X}}$ maps arrays to arrays of the same shape.}  of $\grad_{\mathcal{X}}$,
%on discrete inner product space $X$ (which may be omitted in the notation),
$E_{\beta}^{\alpha}$ is the TPE quadrature defined as (\ref{equ: Tangent-Point Energy Quadrature}),
and $C$ is the discretisation of the constraint energy $\mathcal{C}$ (See section \ref{sct: Constraint Energy}).
...
\end{equation}
where one could explicitly write $\Grad_{L^2} E_{\beta}^\alpha \left( \Gammabf^k \right)$ as
\begin{equation}
    \Grad_{L^2} E_{\beta}^\alpha \left( \Gammabf^k \right)
    =
    \underbrace{
        \begin{pmatrix}
...
One could use this exact form of $\Grad_{L^2} E_{\beta}^{\alpha} \left( \Gammabf^k \right)$ as given in appendix \ref{sct: Exact Gradient}, but it could be considered cumbersome (even though there are benefits to implementing this as stated in section \ref{sct: L2 Complexity}).
One could alternatively approximate $\Grad_{L^2} E_{\beta}^{\alpha} \left( \Gammabf^k \right)$ by central difference scheme, for example:
...
If we \textit{use a difference scheme} to approximate $\Grad_{L^2} E_{\beta}^\alpha \left( \Gammabf^k \right)$,
...
\begin{equation}
    \frac{\Gammabf^{k} - \Gammabf^{k-1}}{\Delta T} = -\Grad_{L^2} E_{\beta}^{\alpha} \left( \Gammabf^k \right)
\end{equation}
...
\subsection{Discussion of Euler Scheme in $H^{-1}$}
...
\subsection{Example: Euler Scheme in $H^1$}
