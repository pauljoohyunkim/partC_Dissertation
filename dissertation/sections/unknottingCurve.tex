...
Represent the initially discretised curve in the tensor\footnote{``Tensors are multidimensional arrays of numerical values and there-
fore generalize matrices to multiple dimensions.''\cite{https://doi.org/10.48550/arxiv.1711.10781} Note that we use the definition from machine learning,
...
$\Grad_{X}$ is discrete equivalent\footnote{Worth noting that $\Grad_X:\mathbb{T} \rightarrow \mathbb{T}$ where $\mathbb{T}$ is a set of tensors of certain shape; $\Grad_X$ maps tensors to tensors of the same shape.}  of $\grad_{\mathcal{X}}$ on discrete inner product space $X$,
...
\subsection{Example: $L^2$ Explicit Euler Scheme for Buck-Orloff Energy}
Now we visit the simplest concrete numerical scheme for curve untangling process.
We will demonstrate with Buck-Orloff energy ($\alpha=2$, $\beta=4$), as one need not take into account of constraint energy $\mathcal{C}$ due to its scale invariance (lemma \ref{lemma: Scale-Invariance})\footnote{One could construct a similar working example for other scale-invariant tangent-point energies.}.


In $L^2$, $\grad_{L^2} \mathcal{E}_{4}^{2} \left( \gammabf \right)$ is simply the ``first-order perturbation'' as in (\ref{equ: L2 Gradient Explicit Form}).
...
\begin{equation}
    \frac{\Gammabf^{k+1} - \Gammabf^k}{\Delta T} = - \Grad_{\ell^2} E_4^2 \left( \Gammabf^k \right)
    \label{equ: L2 Explicit Euler}
\end{equation}
where one could explicitly write $\Grad_{\ell^2} E_{4}^2 \left( \Gammabf^k \right)$ as
\begin{equation*}
    \Grad_{\ell^2} E_{4}^2 \left( \Gammabf^k \right)
    =
    \nabla_{\Gammabf} E_{4}^2 \left( \Gammabf^k \right)
    =
    \begin{pmatrix}
    ...
    \end{pmatrix}
    E_4^2 \left( \Gammabf^k \right)
\end{equation*}
where $x_{j,i}$ refers to the $\left( j,i \right)$ coordinate variable for $3 \times N$ tensor.
Note that $\Grad_{\ell^2} E_{4}^2 \left( \Gammabf^k \right) \in \mathbb{R}^{3 \times N}$ is also a tensor of the same shape as $\Gammabf^k$,
...
One could use this exact form of $\Grad_{\ell^2} E_4^2 \left( \Gammabf^k \right)$ as given in appendix \ref{sct: Exact Gradient}, but it could be considered cumbersome (even though there are benefits to implementing this as stated in section \ref{sct: L2 Complexity}).
One could alternatively approximate $\Grad_{\ell^2} E_{4}^2 \left( \Gammabf^k \right)$ by central difference scheme, for example:
for $i=0,1,\cdots,N-1$ and $j = 1,2,3$,
\begin{equation}
    \ebf_j \cdot \left( \Grad_{\ell^2} E_4^2 \left( \Gammabf^k \right) [i] \right)
    =
    \frac{\partial E_{4}^2 (\Gammabf)}{\partial x_{j,i}}
    \approx
    \frac{1}{2 \Delta X} \left( 
        \bar{E}_{4}^{2} (i)
        -
        \underaccent{\bar} E_{4}^{2} (i)
    \right)
\end{equation}
where
\begin{align*}
    \bar{E}_4^2 (i) &\coloneqq E_{4}^2 \left( \left( \xbf_0, \cdots, \xbf_{i-1}, \xbf_i + \Delta X \ebf_{j}, \xbf_{i+1}, \cdots, \xbf_{N-1}\right) \right) \\
    \underaccent{\bar}{E}_4^2 (i) &\coloneqq E_{4}^2 \left( \left( \xbf_0, \cdots, \xbf_{i-1}, \xbf_i - \Delta X \ebf_{j}, \xbf_{i+1}, \cdots, \xbf_{N-1}\right) \right)
\end{align*}
...
\subsubsection{Explicit Euler Scheme for Scale-variant Energy}
We now attempt the finite difference scheme with a scale-variant energy, that is, when $\beta > \alpha + 2$. 
When one attempts the following finite difference scheme:
\begin{equation}
    \frac{\Gammabf^{k+1} - \Gammabf^k}{\Delta T} = - \Grad_{l^2} E_{\beta}^{\alpha} \left( \Gammabf^k \right)
\end{equation}
...
energy scales inversely proportional to its ``size''.
(See Figure \ref{fig: Scale Variant})
\begin{figure}[tbp]
    \centering
    \includegraphics{sections/unknottingCurveImgs/scaleVariant}
    \caption{The height represents $\mathcal{E}_{4.5}^{2}$ for circles of different radius.}
