...
Represent the initially discretised curve in the tensor form: $\Gammabf^0 = \left( \xbf_0^0, \xbf_1^0, \cdots, \xbf_{N-1}^0 \right)$,
...
$\Grad_{X}$ is discrete equivalent of $\grad_{\mathcal{X}}$ on discrete inner product space $X$,
...
Since the approximation for $\Grad_{L^2} E_\beta^\alpha$ requires evaluation of the energy for each point perturbed, for each time step, it takes $O \left( N^3 \right)$ evaluations of the kernel.
