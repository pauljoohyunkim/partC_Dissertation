...
\begin{enumerate}
    \item Compute $\nabla_{\Gammabf^k} E_{\beta}^{\alpha} \left( \Gammabf^k \right)$.
    \item Solve linear system $\mathcal{L} \mathcal{G} = \nabla_{\Gammabf^k} E_{\beta}^{\alpha} \left( \Gammabf^k \right)$ for $\mathcal{G} \in \mathbb{R}^{3N}$
        \begin{itemize}
            \item $\mathcal{L}$ is the (almost tridiagonal) matrix capturing the discretised curve Laplacian $\curvelaplacian_{\Gammabf^k}$.
            \item $\mathcal{G}$ is the flattening of tensor $\Gammabf^k$ to a vector such that $\mathcal{G} \in \mathbb{R}^{3N \times 1}$.
        \end{itemize}
