...
\end{equation*}
Because solving matrix equation\footnote{``Cyclic tridiagonal matrix'': Can be solved by Gaussian elimination rather efficiently.} $\mathcal{L} \mathcal{G}^T = \left( \nabla_{\Gammabf^k} E_{\beta}^{\alpha} \left( \Gammabf^k \right) \right)$ costs $O \left( N \right)$,
