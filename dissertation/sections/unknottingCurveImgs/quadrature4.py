...
def energy(points, alpha=2, beta=4.5):
    J = len(points)
    ...
    for i in range(J):
        for j in range(J):
            if abs(i - j) > 1 and abs(i - j + J) > 1 and abs(i - j - J) > 1:
                ...
                TI = xI / lI
                kernelAB = 0
                kernelAB += kernelalphabeta(xi, xj, TI, alpha=alpha, beta=beta) 
                kernelAB += kernelalphabeta(xi, xjpm, TI, alpha=alpha, beta=beta) 
                kernelAB += kernelalphabeta(xipm, xj, TI, alpha=alpha, beta=beta) 
                kernelAB += kernelalphabeta(xipm, xjpm, TI, alpha=alpha, beta=beta)
...
def dkalphabeta(curve, p, q, r, k, alpha=3, beta=6, thresholder_enabled=False):
    ...
    xi = thresholder(xi, enabled=thresholder_enabled)
    eta = thresholder(eta, enabled=thresholder_enabled)
    dxi = thresholder(dxi, enabled=thresholder_enabled)
    deta = thresholder(deta, enabled=thresholder_enabled)
